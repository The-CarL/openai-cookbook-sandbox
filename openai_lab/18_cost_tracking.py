...
# Cached input prices follow the standard 10% rule for 4.1/5.4/5.5/5.6 reads.
...
# GPT-5.6 cache writes: billed at 1.25x the uncached input rate (new with 5.6).
# The calculate_cost() below only tracks read-side cached_input; add a
# separate write-cost calculation for 5.6 if you're doing heavy prompt caching.
PRICING = {
    # GPT-5.6 family (GA July 9, 2026) — three durable capability tiers
    # Cache reads at 10% of input; cache WRITES billed at 1.25x input (new).
