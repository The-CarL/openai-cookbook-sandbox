...
# Cached input READ prices follow the standard 10% rule for 4.1/5.4 and 5.5.
...
# GPT-5.6 cache WRITES are billed at 1.25x the input rate (not free); cache reads
# remain ~10% of input. Use explicit prompt_cache_breakpoint to control prefix splits.
PRICING = {
    # GPT-5.6 family (GA July 9, 2026; Luna/Terra prices cut July 30, 2026)
    # cached_input = read price (~10%); cache writes billed separately at 1.25x input
