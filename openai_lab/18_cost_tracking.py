...
# GPT-5.6 uses explicit cache breakpoints (not automatic); cache writes billed
# at 1.25× the input rate; 30-min minimum cache lifetime.
PRICING = {
    # GPT-5.6 family (GA July 9, 2026 — prices reflect July 30 cuts)
    # Luna -80% and Terra -20% on July 30; Sol unchanged.
    # Fast mode (Sol only): service_tier="fast" → 2x price ($10/$60 per 1M).
    "gpt-5.6-sol":   {"input": 5.00, "output": 30.00, "cached_input": None},
    "gpt-5.6-terra": {"input": 2.00, "output": 12.00, "cached_input": None},
    "gpt-5.6-luna":  {"input": 0.20, "output": 1.20,  "cached_input": None},
