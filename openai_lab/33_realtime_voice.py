...
agents via a persistent WebSocket session. Four models serve distinct jobs:
  ...
  gpt-realtime-2.1       — Updated Jul 6, 2026. GPT-5-class reasoning for full
                           voice agents. 25%+ lower p95 latency vs gpt-realtime-2.
                           Adds configurable reasoning effort and improved
                           alphanumeric recognition (order/phone numbers),
                           noise handling, and interruption detection.
                           Billing: text in $4/M, audio in $32/M,
                                    text out $8/M, audio out $64/M.
  gpt-realtime-2.1-mini  — Jul 6, 2026. Smaller reasoning model: faster and
                           lower-cost for latency-sensitive voice pipelines.
                           Billing: text in $0.60/M, audio in $10/M,
                                    text out $2.40/M, audio out $20/M.
...
──────────────────────────────────────────────────────────────────────
gpt-realtime-2.1          Full voice agent, reasoning,      text in $4/M
                          function calls, interruption       audio in $32/M
                          handling. 25%+ lower latency       text out $8/M
                          vs gpt-realtime-2 (Jul 6, 2026).  audio out $64/M

gpt-realtime-2.1-mini     Faster/cheaper voice pipeline.    text in $0.60/M
                          Smaller reasoning model.           audio in $10/M
                          (Jul 6, 2026)                      audio out $20/M
...
Key differences from the Responses API:
  ...
  ✓ Configurable reasoning effort (new in 2.1)
...
When to use which:
  ...
  gpt-realtime-2.1-mini  — Same use cases where cost/latency matter more
