...
agents via a persistent WebSocket session. Three model families serve distinct jobs:
  ...
  gpt-realtime-2.1.1       — GPT-5-class reasoning for full voice agents.
                           July 6, 2026: cuts p95 latency ≥25% vs gpt-realtime-2.1.
                           Billing: audio tokens (input ~$32/M, output ~$64/M).
  gpt-realtime-2.1.1-mini  — Reasoning + tool use at mini cost (~$10/$20/M audio).
                           Same price as the prior gpt-realtime-mini.
...
async def example_1_text_convo():
    ...
    print("(Use gpt-realtime-2.1-mini for the same features at ~3× lower cost)")
    print()
...
gpt-realtime-2.1-mini    Reasoning + tool use at mini cost    Audio tokens
                         Same capabilities as 2.1, lower      (~$10/$20/M)
                         cost for high-volume voice workloads
...
When to use which:
  ...
  gpt-realtime-2.1-mini  — Same as 2.1 but ~3× cheaper; good default for
                           cost-sensitive high-volume voice workloads.
