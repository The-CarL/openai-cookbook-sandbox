...
agents via a persistent WebSocket session. Models (updated July 6, 2026):

  gpt-realtime-2.1       — GPT-5-class reasoning for full voice agents.
                           Billing: audio tokens (input $32/M, output $64/M).
                           vs gpt-realtime-2: improved alphanumeric recognition,
                           silence/noise handling, interruptions; 25% p95 latency drop.
  gpt-realtime-2.1-mini  — Reasoning + tool use at mini price (new July 6, 2026).
...
gpt-realtime-2.1-mini    Reasoning + tool use at mini cost    Audio tokens
  (Jul 6, 2026)          Lower latency, cheaper tier          (in $10/M,
                                                              out $20/M)
...
Key differences from the Responses API:
  ...
  ✓ Function calling works in gpt-realtime-2.1 and 2.1-mini
...
When to use which:
  ...
  gpt-realtime-2.1-mini  — Same capabilities at 3× lower input cost;
                           choose when latency matters more than peak quality.
