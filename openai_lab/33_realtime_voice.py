...
agents via a persistent WebSocket session. Three model families serve distinct jobs:
  ...
  gpt-realtime-2.1.1       — GA July 6, 2026. GPT-5-class reasoning for full voice agents.
                           25% lower p95 latency vs gpt-realtime-2.1. Better alphanumeric
                           recognition and silence/noise handling.
                           Billing: text in $4/M, audio in $32/M, audio out $64/M.
                           Cached audio: $0.40/M (~99% discount on audio — design for it).
  gpt-realtime-2.1.1-mini  — Budget tier. Audio in $10/M, audio out $20/M.
                           Cached audio: $0.40/M (same 99% discount as full model).
...
def summary():
    ...
    print("=" * 60)
    print("""
Model                   Use case                            Billing (per 1M tokens)
──────────────────────────────────────────────────────────────────────────────────
gpt-realtime-2.1        Full voice agent with GPT-5          Text in $4, audio in $32
                        reasoning, function calls,           Audio out $64
                        interruption handling                Cached audio $0.40 (~99% off)

gpt-realtime-2.1-mini   Same capabilities at lower cost      Audio in $10, audio out $20
                        Ideal for high-volume deployments    Cached audio $0.40
...
When to use which:
  ...
  gpt-realtime-2.1-mini   — Same use cases at ~3× lower audio cost; start
                            here for new projects and step up if quality gaps.
...
Prompt caching:
  gpt-realtime-2.1 (and mini) support prompt caching on audio tokens.
  Cached audio rate: $0.40/M on both tiers — ~99% discount.
  Design your system prompt and static audio prefix to hit this aggressively.
