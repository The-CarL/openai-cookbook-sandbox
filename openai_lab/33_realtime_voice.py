...
agents via a persistent WebSocket session. Three models serve distinct jobs:
  ...
  gpt-realtime-2.1       — GPT-5-class reasoning for full voice agents (Jul 6, 2026).
                           25%+ lower p95 latency vs gpt-realtime-2; better alphanumeric
                           recognition, noise handling, and interruption behaviour.
                           Billing: audio tokens (input ~$32/M, output ~$128/M).
  gpt-realtime-2.1-mini  — Mini reasoning variant: lower cost, faster responses.
                           For high-volume or latency-sensitive voice workloads.
...
Note: gpt-realtime-2 deprecated July 20, 2026 (shutdown Jan 20, 2027).
...
gpt-realtime-2.1-mini     High-volume or latency-sensitive     Audio tokens
                          voice apps; 70% cheaper than 2.1    (in ~$10/M,
                                                               out ~$20/M)
...
When to use which:
  ...
  gpt-realtime-2.1-mini   — Same use cases at lower cost; trade some
                            capability for speed and price.
