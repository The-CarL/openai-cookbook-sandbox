...
agents via a persistent WebSocket session.

Update July 6, 2026: gpt-realtime-2.1 and gpt-realtime-2.1-mini released.
  - 25%+ reduction in p95 latency vs gpt-realtime-2
  - Adds configurable reasoning and tool use
  - Better alphanumeric recognition, silence/noise handling, interruption behaviour

Models:

  gpt-realtime-2.1       — Full voice agent with configurable reasoning + tool use.
                           Billing: audio tokens (input $32/M, output $64/M).
  gpt-realtime-2.1-mini  — Lower-cost voice agent; same reasoning/tool features.
...
───────────────────────────────────────────────────────────────────────
gpt-realtime-2.1          Full voice agent; configurable      Audio tokens
  (Jul 6, 2026)           reasoning + tool use; 25%+ lower   (in $32/M,
                          p95 latency vs 2.0                  out $64/M)

gpt-realtime-2.1-mini     Same reasoning/tool features at     Audio tokens
  (Jul 6, 2026)           lower cost; best for high-volume    (in $10/M,
                          voice pipelines                     out $20/M)
...
When to use which:
  gpt-realtime-2.1        — Customer support bots, voice assistants, any
                            ...
                            interruptions in real time. Use mini for volume.
