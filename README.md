...
## Model lineup snapshot (verified July 28, 2026)
...
| `gpt-5.4-mini` | 0.75 | 4.50 | 400K | Solid agentic default; compare against 5.6-luna for new workloads |
| `gpt-5.4` | 2.50 | 15.00 | 1M | Computer use, image gen, compaction; same price as 5.6-terra |
| `gpt-5.4-pro` | — | — | 1M | March 5: computationally intensive problems |
| `gpt-5.5` | 5.00 | 30.00 | 1M | Apr 24. Long-context gotcha: >272K billed at 2x/1.5x per token |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | **New flagship (GA Jul 9)**. Strongest coding, reasoning, cybersecurity |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1.05M | Balanced 5.6 tier; same price as 5.4 but stronger across the board |
| `gpt-5.6-luna` | 1.00 | 6.00 | 1.05M | Budget 5.6 tier; fast, affordable for high-volume applications |
...
- **GPT-5.6 cache writes are billed at 1.25× the uncached input rate** (reads still 90% off). Minimum cache lifetime is 30 minutes.
...
- **GPT-5.6 family** (GA July 9, 2026) — in model lineup table and PRICING dicts (ex. 04, 18). Sol ($5/$30/M), Terra ($2.50/$15/M), Luna ($1/$6/M). Cache writes billed at 1.25× input; 30-min minimum cache lifetime. New naming: number = generation, Sol/Terra/Luna = durable capability tiers
