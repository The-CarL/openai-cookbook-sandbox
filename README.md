...
- **`gpt-realtime-2`** (May 7) — Realtime voice model with GPT-5-class reasoning; replaces `gpt-realtime-1.5`. Adjustable reasoning effort (minimal–very high, default low), 128K context window. $32/1M audio-in, $64/1M audio-out
- **`gpt-realtime-translate`** (May 7) — Live streaming speech translation: 70+ input languages → 13 output languages ($0.034/min)
