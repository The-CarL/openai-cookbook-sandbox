...
## Model lineup snapshot (verified July 24, 2026)
...
| `gpt-5.6-luna` | 1.00 | 6.00 | 1.05M | Jul 9: cost-optimized 5.6. Budget agentic work |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1.05M | Jul 9: balanced 5.6. Strong coding + reasoning |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | Jul 9: 5.6 flagship. Best coding, cybersecurity, complex reasoning |
...
- **GPT-5.6 introduces explicit cache breakpoints** (up to 4 writes per request, reads back 50 breakpoints). Cache writes cost **1.25× the input rate**; cache reads remain 10% of input. Minimum cache lifetime is 30 minutes.
...
- **GPT-5.6 family** (GA July 9, 2026) — Sol ($5/$30/M), Terra ($2.50/$15/M), Luna ($1/$6/M); 1.05M context. Stronger reasoning, coding, and cybersecurity vs 5.5. Introduces explicit cache breakpoints (30-min minimum lifetime, writes billed at 1.25× input). Model IDs: `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` (alias `gpt-5.6` → Sol). Added to ex. 04 + 18; a dedicated exercise (35) covering the new caching model is a natural next step
