...
## Model lineup snapshot (verified July 23, 2026)
...
| `gpt-5.4-mini` | 0.75 | 4.50 | 400K | Agentic workloads; gpt-5.6-luna now competes at similar price |
| `gpt-5.4` | 2.50 | 15.00 | 1M | Consider gpt-5.6-terra — same price, stronger model |
| `gpt-5.4-pro` | — | — | 1M | March 5: computationally intensive problems |
| `gpt-5.5` | 5.00 | 30.00 | 1M | Superseded by 5.6-sol (same price, better) and 5.6-terra (half price) |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-luna` | 1.00 | 6.00 | 1M | **NEW Jul 9.** Fast + cheap; high-volume production default |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1M | **NEW Jul 9.** Same price as 5.4, stronger; new default for most tasks |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1M | **NEW Jul 9.** Current flagship; outperforms GPT-5.5 at same price |
...
### Caching gotchas
- Cached input reads are ~10% of standard input across all GPT families.
- Verify hits via `usage.input_tokens_details.cached_tokens` (Exercise 25).
- **GPT-5.6 only: cache WRITES are billed at 1.25× the input rate** (new vs earlier families). Cache reads remain 10%.
...
- **GPT-5.6 family** (GA July 9, 2026) — Sol/Terra/Luna tiers; programmatic tool calling (model writes JS to orchestrate tools in a V8 sandbox), persisted reasoning across turns (`reasoning.context`), max reasoning effort level, multi-agent orchestration beta, explicit prompt cache breakpoints. Exercise 35 planned.
- **`gpt-realtime-2.1` / `gpt-realtime-2.1-mini`** (July 6, 2026) — drop-in upgrade from gpt-realtime-2; 25% lower p95 latency, same pricing. Mini adds reasoning + tool use. Upgrade ex. 33 to reference 2.1 when exercising voice agents.
