...
## Model lineup snapshot (verified July 15, 2026)
...
| `gpt-5.6-luna` | 1.00 | 6.00 | 1M | Jul 9 GA. Fast everyday tier. Good for high-volume 5.6 workloads |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1M | Jul 9 GA. Balanced 5.6 tier, same per-token price as 5.4 |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1M | Jul 9 GA. Flagship (alias: `gpt-5.6`). Hardest coding/cyber/agents |
...
### Caching gotchas
- Cached input is ~10% of standard input across all GPT families.
...
- **GPT-5.6 cache writes are billed at 1.25× the uncached input rate** (new with 5.6). Cache reads remain at the standard 90% discount. GPT-5.6 also supports explicit cache breakpoints with a 30-minute minimum cache lifetime.
...
- **Programmatic Tool Calling** (GA July 9, 2026, with GPT-5.6) — model writes JavaScript that runs in an isolated V8 runtime to compose tool calls in one round-trip; ZDR-compatible. Tools opt in via `allowed_callers`.
- **Multi-agent Responses API** (beta July 9, 2026, GPT-5.6 only) — `multi_agent={"enabled": True, "max_concurrent_subagents": N}` lets the root model spawn and synthesize parallel subagents in a single request.
