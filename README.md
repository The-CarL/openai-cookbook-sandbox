...
- **Realtime API — now GA (May 7, 2026)**: three new real-time voice models:
  - **`gpt-realtime-2`** — GPT-5-class reasoning voice model; 128K context, adjustable reasoning effort; $32/M audio input ($0.40 cached), $64/M audio output
  - **`gpt-realtime-translate`** — streaming speech translation, 70+ input → 13 output languages; $0.034/min
  - **`gpt-realtime-whisper`** — streaming speech-to-text transcription; $0.017/min
- **`gpt-audio-1.5`** (Feb 23) — Chat Completions audio model (REST-based audio input/output; separate from Realtime API)
