...
## Model lineup snapshot (verified August 21, 2026)
...
| `gpt-5.6-luna` | 0.20 | 1.20 | 1.05M | Jul 9 GA. Budget tier; cache writes at 1.25× input |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1.05M | Jul 9 GA. Balanced everyday work |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | Jul 9 GA. Frontier: coding, cybersecurity, science |
...
### Caching gotchas
- Cached input is ~10% of standard input across GPT-4.1 and GPT-5.4/5.5 families.
...
- **GPT-5.6 cache model**: explicit cache breakpoints, 30-min minimum cache lifetime, cache *writes* billed at 1.25× input rate (unlike other families where writes are free).
...
- **GPT-5.6 family** (GA Jul 9, 2026; price cuts Jul 30) — gpt-5.6-sol ($5/$30/M), gpt-5.6-terra ($2/$12/M, −20%), gpt-5.6-luna ($0.20/$1.20/M, −80%); 1.05M context, 128K max output. Now in the model lineup table above. See ex04 and ex18 for pricing integration. **Programmatic Tool Calling** (model-written JS in an isolated V8 runtime) — new built-in; exercise pending.
