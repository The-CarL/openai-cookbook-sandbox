...
## Model lineup snapshot (verified August 9, 2026)
...
| `gpt-5.6-luna` | 0.20 | 1.20 | 1.05M | GA Jul 9; cut 80% Jul 30 (was $1/$6). Budget 5.6: coding, cybersec |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1.05M | GA Jul 9; cut 20% Jul 30 (was $2.50/$15). New mid-tier default |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | GA Jul 9. Frontier: best-in-class coding, reasoning, agentic work |
...
- **GPT-5.6 introduces a new cache billing model**: cache writes are billed at **1.25× the input rate** (e.g. $0.25/M for luna). Use `prompt_cache_key` for explicit 30-min cache breakpoints. Cache write cost appears as a separate line item (not in `usage`) — `calculate_cost()` in ex. 18 does not capture it.
...
- **GPT-5.6 family** (GA July 9, 2026; Luna/Terra prices cut July 30) — now in exercises 04 and 18. Sol ($5/$30/M), Terra ($2/$12/M), Luna ($0.20/$1.20/M). 1.05M context. Cache writes billed at 1.25× input; `prompt_cache_key` sets explicit 30-min breakpoints. Fast mode for Sol: 2.5× faster at 2× price (replaces Priority Processing).
