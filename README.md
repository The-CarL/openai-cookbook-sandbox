...
## Model lineup snapshot (verified July 25, 2026)
...
| `gpt-5.5` | 5.00 | 30.00 | 1M | Apr 24 flagship. Still strong; prefer 5.6-sol for new workloads |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-luna` | 1.00 | 6.00 | 1M | GA Jul 9. Fastest/cheapest 5.6; cost-sensitive volume |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1M | GA Jul 9. Balanced; GPT-5.5-competitive at 2× lower token price |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1M | GA Jul 9. New frontier flagship; stronger reasoning, coding, cybersec |
...
### Caching gotchas
- Cached input is ~10% of standard input across the GPT-4.1 and GPT-5.4/5.5 families.
...
- **GPT-5.6 uses explicit cache breakpoints** (via `prompt_cache_breakpoint`). Cache writes are billed at 1.25× the input rate; reads keep the 90% discount. Minimum cache TTL is 30 minutes.
...
- **GPT-5.6 family** (GA July 9, 2026) — `gpt-5.6-sol` ($5/$30/M), `gpt-5.6-terra` ($2.50/$15/M), `gpt-5.6-luna` ($1/$6/M). Now in model table and pricing dicts (ex. 04 + 18). Caching model changed: explicit breakpoints via `prompt_cache_breakpoint`, 30-min TTL, writes billed at 1.25× input. Long-context meter: Sol $10/$45, Terra $5/$22.50, Luna $2/$9 for sessions >272K tokens
