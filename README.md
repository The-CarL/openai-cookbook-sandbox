...
| 33 | Realtime API v2.1 (Jul 2026) | `gpt-realtime-2.1` / 2.1-mini / translate / whisper WebSocket voice agents |
...
- **`gpt-audio-1.5`** (Feb 23) — Chat Completions audio model. **Deprecation notice Jul 20, 2026**: legacy audio, realtime, and transcription model snapshots are deprecated; removal Jan 20, 2027. Migrate to gpt-realtime-2.1.
- **`gpt-realtime-2.1` / `gpt-realtime-2.1-mini`** (Jul 6, 2026) — updated by ex. 33. Mini adds a lower-cost distilled reasoning tier ($10/$20/M audio). `return_token_budget` parameter also added to the web_search tool for high-effort research runs.
