...
## Model lineup snapshot (verified August 1, 2026)
...
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | New frontier flagship (Jul 9). Best coding, science, cybersecurity |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1.05M | Balanced everyday work. 20% price cut Jul 30 (was $2.50/$15) |
| `gpt-5.6-luna` | 0.20 | 1.20 | 1.05M | Fastest/cheapest 5.6 tier. 80% price cut Jul 30 (was $1/$6) |
...
- **GPT-5.6 family**: cache reads are 90% off (same 10% rule); cache *writes* cost **1.25× input** with a 30-min minimum cache lifetime. Sol supports a Fast mode (2.5× speed, 2× price) via `service_tier: "fast"`.
...
- **GPT-5.6 family** (GA July 9, 2026; price cuts July 30) — Sol ($5/$30), Terra ($2/$12, was $2.50/$15), Luna ($0.20/$1.20, was $1/$6). All 1.05M context. Caching: reads 90% off, writes 1.25× input; 30-min min cache lifetime. Sol: `service_tier: "fast"` for 2.5× speed at 2× price. **Now in exercises 04 and 18.**
- **Fast mode** (July 30, 2026) — replaces Priority Processing. Available for gpt-5.6-sol initially. Pass `service_tier: "fast"` in the Responses API.
