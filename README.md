...
## Model lineup snapshot (verified May 7, 2026)
...
| `gpt-5.5` | 5.00 / **10.00**¹ | 30.00 / **45.00**¹ | 1M | New flagship (Apr 24). Token-efficient → often cheaper end-to-end |
...
¹ **GPT-5.5 long-context surcharge**: sessions with >272K input tokens are billed at **2× input ($10.00/M) and 1.5× output ($45.00/M) for the entire session** — the surcharge applies to all tokens once the threshold is crossed, not just the excess.
...
- **GPT-5.5 long-context pricing**: >272K input tokens triggers 2× input + 1.5× output rates for the full session (see ¹ above). Plan context budgets accordingly.
...
- **`gpt-realtime`** — Realtime API voice model (GA; Realtime API Beta deprecated May 7, 2026)
