...
## Model lineup snapshot (verified September 4, 2026)
...
| `gpt-5.6-luna` | 0.20 | 1.20 | — | GA Jul 9; price cut 80% on Jul 30. Cost-efficient 5.6 tier |
| `gpt-5.6-terra` | 2.00 | 12.00 | — | GA Jul 9; price cut 20% on Jul 30. Balanced 5.6 tier |
| `gpt-5.6-sol` | 4.00 | 20.00 | — | GA Jul 9; promo price (cut Aug 21) through Nov 21, 2026. 5.6 flagship |
...
- **GPT-5.6 uses explicit cache breakpoints** (not automatic). 30-min minimum cache lifetime. Cache *writes* billed at 1.25× input rate; reads at ~10% (standard discount still applies to reads).
...
- **GPT-5.6 family** (GA July 9, 2026; covered in ex. 04 + 18) — Sol, Terra, Luna tiers. Introduces explicit cache breakpoints with 30-min minimum cache lifetime; cache writes billed at 1.25× input rate. **Price cuts**: Luna −80% and Terra −20% on Jul 30; Sol cut ~20%/33% on Aug 21 (promotional through Nov 21, 2026). A new exercise (35) covering the caching model differences from earlier families would be valuable
