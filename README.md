...
| 33 | Realtime API v2.1 (Jul 2026) | `gpt-realtime-2.1` / mini / translate / whisper; reasoning effort, −25% p95 latency |
| 34 | Inline moderation (Jun 2026) | Safety scores alongside `responses.create()` in one call |
| 35 | Programmatic Tool Calling (Jul 2026) | GPT-5.6 writes JS to coordinate tools in V8; 38–64% fewer tokens |

## Model lineup snapshot (verified July 11, 2026)
...
| `gpt-5.6-luna` | 1.00 | 6.00 | 1M | **GA Jul 9.** Fastest/cheapest GPT-5.6. Cache writes 1.25×, 30-min min |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1M | **GA Jul 9.** Balanced GPT-5.6; same token price as 5.4, newer gen |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1M | **GA Jul 9.** Flagship. Programmatic TC; top reasoning, coding, cybersec |
...
- **GPT-5.6 cache writes are billed at 1.25× standard input rate** (a new charge vs earlier models). Minimum cache lifetime is 30 minutes. Cache reads remain at 10% of standard.
...
- **GPT-5.6 family** (GA July 9, 2026) — Sol/Terra/Luna in model table above; Programmatic Tool Calling covered by ex. 35. GPT-Live-1 and GPT-Live-1 mini full-duplex voice models (Jul 8) are ChatGPT-only for now; API access planned — watching.
