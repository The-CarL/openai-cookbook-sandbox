...
## Model lineup snapshot (verified August 8, 2026)
...
| `gpt-5.6-luna` | 0.20 | 1.20 | 1M | Fastest/cheapest 5.6; high-volume. Price cut 80% Jul 30. |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1M | Balanced everyday 5.6. Price cut 20% Jul 30. |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1M | 5.6 flagship: strongest reasoning, coding, agentic work. GA Jul 9. |
...
- **GPT-5.6 uses explicit cache breakpoints** (not the 10% rule): 30-min minimum cache lifetime; cache writes billed at 1.25× the input rate.
...
- **GPT-5.6 family** (GA July 9, 2026; price cuts July 30) — `gpt-5.6-sol` ($5/$30/M, unchanged), `gpt-5.6-terra` ($2/$12/M, was $2.50/$15), `gpt-5.6-luna` ($0.20/$1.20/M, was $1/$6). Caching: explicit breakpoints, 30-min minimum, writes at 1.25× input rate (different from 4.1/5.4/5.5 10% rule). Now in model lineup table above.
- **Fast mode for gpt-5.6-sol** (July 30, 2026) — `service_tier: "fast"` delivers up to 2.5× the speed of standard processing at 2× the price ($10/$60/M). Replaces the legacy `priority` value (still accepted for backward compatibility).
