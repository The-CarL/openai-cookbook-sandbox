...
| 35 | GPT-5.6 caching (Jul 2026) | `prompt_cache_key` + `prompt_cache_options` + 1.25× write billing |

## Model lineup snapshot (verified July 17, 2026)
...
| `gpt-5.6-luna` | 1.00 | 6.00 | — | Jul 9 GA: efficient tier — cheapest 5.x, great for routing + high-volume |
| `gpt-5.6-terra` | 2.50 | 15.00 | — | Jul 9 GA: balanced tier — same price as 5.4, stronger reasoning |
| `gpt-5.6-sol` | 5.00 | 30.00 | — | Jul 9 GA: frontier tier — matches 5.5 price, stronger reasoning |
...
- **GPT-5.6 cache writes are billed at 1.25× input rate** — unlike all earlier families where writes were free. Use `prompt_cache_key` + `prompt_cache_options` for reliable matching. See Exercise 35.
...
- **GPT-5.6 family** (GA July 9, 2026) — Sol ($5/$30/M), Terra ($2.50/$15/M), Luna ($1/$6/M). **Covered by ex. 35** (caching) and the model lineup table above. Key new API: `prompt_cache_key`, `prompt_cache_options.mode/ttl`, `cache_write_tokens` in usage. Cache writes billed 1.25×.
