...
## Model lineup snapshot (verified August 2, 2026)
...
| `gpt-5.4-mini` | 0.75 | 4.50 | 400K | Default for agentic workloads not yet on 5.6. Tool search, computer, compaction |
| `gpt-5.4` | 2.50 | 15.00 | 1M | Cheaper than 5.5/5.6; computer use, image gen, native compaction |
| `gpt-5.4-pro` | — | — | 1M | March 5: computationally intensive problems |
| `gpt-5.5` | 5.00 | 30.00 | 1M | Apr 24 flagship. Consider 5.6-terra at comparable price |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-luna` | 0.20 | 1.20 | — | GA Jul 9; price cut 80% on Jul 30. Ultra-cheap routing/triage |
| `gpt-5.6-terra` | 2.00 | 12.00 | — | GA Jul 9; price cut 20% on Jul 30. New everyday-work default |
| `gpt-5.6-sol` | 5.00 | 30.00 | — | GA Jul 9; alias `gpt-5.6`. Hardest tasks. Fast mode: 2.5× at 2× price |
...
- **GPT-5.6 explicit caching** (`prompt_cache_options.mode="explicit"`): cache writes cost 1.25× input; reads stay at 10%; 30-min minimum TTL. Developer-placed breakpoints via `prompt_cache_breakpoint`.
...
- **GPT-5.6 family** (GA July 9, 2026; pricing updated) — Sol/Terra/Luna added to ex. 04 and 18. New API features not yet exercised: **Fast mode** (`reasoning.mode`-like flag; 2.5× speed at 2× price for Sol), **explicit prompt caching** (`prompt_cache_options.mode="explicit"`, cache writes at 1.25×), **Pro mode** (`reasoning.mode: "pro"` on all three variants), **six-level reasoning effort** (`none` through `max`). Candidate for exercise 35.
- **GPT Transcribe / GPT Live Transcribe** (Jul 28, 2026) — `gpt-transcribe` (async file transcription) and `gpt-live-transcribe` (Realtime API, low-latency streaming). Improve on Whisper across accents, background noise, short phrases, numbers. Candidate for a new exercise.
