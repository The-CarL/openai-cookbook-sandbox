...
## Model lineup snapshot (verified August 17, 2026)
...
| `gpt-5.6-luna` | 0.20 | 1.20 | 1.05M | GA Jul 9; price cut Jul 30 (−80%). High-volume routing |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1.05M | GA Jul 9; price cut Jul 30 (−20%). Balanced everyday production |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | GA Jul 9. Flagship; `gpt-5.6` aliases Sol. 128K max output |
...
### Caching gotchas
- Cached input is ~10% of standard input across the GPT families (including 5.6).
...
- GPT-5.6 introduces explicit cache breakpoints (30-min minimum cache lifetime); cache writes are billed at 1.25× input rate.
...
- **GPT-5.6 new Responses API surface** (GA Jul 9) — Programmatic Tool Calling (V8 sandbox, 38–63.5% token reduction), `reasoning.effort: max`, persisted reasoning across turns, native multi-agent orchestration (beta). No exercise yet; watching for API reference to stabilize.
