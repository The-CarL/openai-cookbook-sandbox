...
## Model lineup snapshot (verified July 20, 2026)
...
| `gpt-5.4-mini` | 0.75 | 4.50 | 400K | Agentic workloads where 5.6 is budget-constrained |
...
| `gpt-5.5` | 5.00 | 30.00 | 1M | Still strong; token-efficient. Long-context gotcha >272K |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-luna` | 1.00 | 6.00 | 1M | GA Jul 9. High-volume; fastest/cheapest 5.6 tier |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1M | GA Jul 9. General workloads, balanced intelligence/cost |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1M | GA Jul 9. New frontier flagship; explicit caching (1.25× write) |
...
### Caching gotchas
- Cached input is ~10% of standard input across the GPT families (90% read discount).
...
- **GPT-5.6 uses explicit caching.** Cache reads remain 90% off, but cache **writes** are now billed at **1.25× the uncached input rate** (e.g. Sol writes cost $6.25/M vs $5.00/M input). Explicit breakpoints (up to 4/request) replace implicit caching; minimum TTL is 30 min.
...
- **GPT-5.6 family** (GA July 9, 2026) — `gpt-5.6-sol` ($5/$30/M), `gpt-5.6-terra` ($2.50/$15/M), `gpt-5.6-luna` ($1/$6/M); 1M context, 128K max output, Feb 16 2026 knowledge cutoff. Explicit caching model: reads still 90% off, writes billed at 1.25× input. Pricing and models added to exercises 04 and 18. A dedicated exercise on the new caching model is a logical next step.
