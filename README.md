...
## Model lineup snapshot (verified July 30, 2026)
...
| `gpt-5.4-mini` | 0.75 | 4.50 | 400K | Solid default for agentic workloads; lower cost than 5.6-terra |
| `gpt-5.4` | 2.50 | 15.00 | 1M | Cost-sensitive flows where 5.6 would be overkill |
| `gpt-5.4-pro` | — | — | 1M | March 5: computationally intensive problems |
| `gpt-5.5` | 5.00 | 30.00 | 1M | Apr 24 flagship. Same price as 5.6-sol but 5.6-sol is newer |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | **Jul 9 GA flagship.** `gpt-5.6` aliases here. Coding, research, computer use |
| `gpt-5.6-terra` | 2.50 | 15.00 | 1.05M | Balanced quality/cost; replaces 5.4 as everyday workhorse |
| `gpt-5.6-luna` | 1.00 | 6.00 | 1.05M | Cost-focused; high-volume tasks that don't need Sol-level quality |
...
- **GPT-5.6 cache writes are billed at 1.25× the input rate** (cache reads remain 10%). The write premium appears on your invoice, not in the API response. See pricing comment in ex. 18.
...
- **GPT-5.6 family** (GA July 9, 2026) — `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-luna` (`gpt-5.6` aliases to Sol). 1.05M context, 128K max output. Sol/Terra/Luna pricing in lineup table. Explicit cache breakpoints, 30-min cache TTL, 1.25× write premium. Now in ex. 04 and 18; no dedicated exercise yet
