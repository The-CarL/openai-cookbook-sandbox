...
## Model lineup snapshot (verified August 10, 2026)
...
| `gpt-5.4-mini` | 0.75 | 4.50 | 400K | Agentic workloads when 5.6-luna is overkill |
...
| `gpt-5.5` | 5.00 | 30.00 | 1M | Apr 24. Token-efficient; now between 5.4 and 5.6 in the lineup |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-luna` | 0.20 | 1.20 | 1M | GA Jul 9 (80% cut Jul 30). Ultra-fast; replaces 5.4-mini for volume |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1M | GA Jul 9 (20% cut Jul 30). Balanced; competitive with gpt-5.5 |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1M | GA Jul 9. New frontier: stronger reasoning, coding, cybersec |
...
- **GPT-5.6 adds explicit cache breakpoints** — you mark boundaries yourself; 30-min minimum cache lifetime. Cache reads remain 10% of input. Cache *writes* are billed at 1.25× the standard input rate (new in 5.6).
...
- **GPT-5.6 family** (GA Jul 9, 2026; covered in ex. 04 model comparison) — `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-luna`. Jul 30 price cuts: luna -80% ($0.20/$1.20/M), terra -20% ($2.00/$12.00/M), sol unchanged ($5/$30/M). Explicit cache breakpoints + 1.25× cache-write pricing covered in caching gotchas above. Worth a dedicated ex. 35 for the new caching mechanics.
