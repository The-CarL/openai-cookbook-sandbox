...
## Model lineup snapshot (verified August 14, 2026)
...
| `gpt-5.6-luna` | 0.20 | 1.20 | 1.05M | GA Jul 9; cheapest 5.6 tier after Jul 30 −80% price cut |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1.05M | GA Jul 9; balanced. Jul 30 −20% cut. Replaces 5.5 as mid-tier |
| `gpt-5.6-sol` | 5.00 | 30.00 | 1.05M | GA Jul 9; frontier capability (same price as gpt-5.5) |
...
- **GPT-5.6 uses explicit cache breakpoints** — set `prompt_cache_breakpoint=True` in content blocks. 30-min minimum TTL. Cache reads: 90% discount (same as other families). Cache writes: billed at 1.25× input rate (new charge type, not present in older models).
...
- **GPT-5.6 family** — GA July 9, 2026 (13 days after Jun 26 preview). Price drop Jul 30: Luna −80% ($0.20/$1.20/M), Terra −20% ($2.00/$12.00/M), Sol unchanged ($5/$30/M). Now in model lineup table above. Explicit cache breakpoints; cache writes at 1.25× input. Cybersecurity variant `gpt-5.6-cyber` released Aug 2026 for authorized defenders
