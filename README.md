...
- **`gpt-realtime-2`** (May 8) — GPT-5-class reasoning voice model; 128K context (up from 32K); configurable reasoning effort (minimal/low/medium/high/very high, default low); $32/M audio input, $64/M audio output. **Replaces `gpt-realtime-1.5`. Realtime API Beta interface deprecated and removed May 12, 2026 — use GA Realtime API.**
- **`gpt-realtime-translate`** (May 11) — streaming speech translation, 70+ input languages → 13 output languages
- **`gpt-realtime-whisper`** (May 11) — streaming speech-to-text for live captioning and post-call workflows
