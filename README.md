...
- **`gpt-realtime-2`** (May 7) — Realtime API voice model with GPT-5-class reasoning; 128K context; $32/$64 per 1M audio in/out. Also launched: `gpt-realtime-translate` (streaming 70+ input langs → 13 output langs, $0.034/min) and `gpt-realtime-whisper` (live STT, $0.017/min). Supersedes `gpt-realtime-1.5`.
...
- **Realtime API** (voice / audio streaming) — gpt-realtime-2 family is the current lineup (see above)
