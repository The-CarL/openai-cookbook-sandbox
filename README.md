...
## Model lineup snapshot (verified August 30, 2026)
...
| `gpt-5.4-mini` | 0.75 | 4.50 | 400K | Agentic workloads; tool search, computer use, compaction |
| `gpt-5.4` | 2.50 | 15.00 | 1M | Computer use, image gen, native compaction; cheaper than 5.5 |
| `gpt-5.4-pro` | — | — | 1M | March 5: computationally intensive problems |
| `gpt-5.5` | 5.00 | 30.00 | 1M | Apr 24 flagship (now surpassed by 5.6). Token-efficient vs 5.4 |
| `gpt-5.5-pro` | 30.00 | 180.00 | 1M | Hardest reasoning, unchanged from 5.4 Pro pricing |
| `gpt-5.6-luna` | 0.20 | 1.20 | 1.05M | Cheapest 5.6 tier; GA Jul 9 |
| `gpt-5.6-terra` | 2.00 | 12.00 | 1.05M | Balanced; new default for agentic workloads; GA Jul 9 |
| `gpt-5.6-sol` | 4.00 | 20.00 | 1.05M | Current flagship; promo rate through Nov 21 (was $5/$30); GA Jul 9 |
...
### Caching gotchas
- Cached input is ~10% of standard input across the GPT families (reads).
...
- **GPT-5.6 cache writes are billed at 1.25× the input rate** (new vs prior families). Cache reads still get the 10% discount. Models also support explicit cache breakpoints with a 30-min minimum cache lifetime.
...
- **GPT-5.6 family** (GA July 9, 2026) — `gpt-5.6-sol` ($4/$20/M promotional through Nov 21; alias `gpt-5.6`), `gpt-5.6-terra` ($2/$12/M), `gpt-5.6-luna` ($0.20/$1.20/M); 1.05M context; stronger reasoning, coding, and cybersecurity. Cache writes billed at 1.25× input rate; reads at 10% discount. Explicit cache breakpoints + 30-min min cache lifetime. *(Prices cut July 30 for Terra/Luna; Sol cut again Aug 21)*
